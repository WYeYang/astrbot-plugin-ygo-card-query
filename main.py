    ...
    async def query_card(self, event: AstrMessageEvent, sql: str = ""):
        ...
        Args:
            sql(string): SQL查询语句。格式：SELECT * FROM datas d JOIN texts t ON d.id=t.id WHERE ...
                ...
                - d.level: 等级/阶级/链接数
                - d.atk/d.def: 攻击力/防御力
                提示：查单张卡用 ORDER BY RANDOM() LIMIT 1；查多张用 ORDER BY RANDOM() LIMIT 3；搜效果用 t.desc LIKE '%关键词%'；查询时请使用简体中文关键词；此工具只需调用一次即可获取完整信息；如果用户想找"老婆"或妹卡，建议搜索：黑魔导女孩、青眼少女、闪刀姬、半龙女仆、龙女仆、虫惑魔、直播双子、邪恶双子、黄金卿、灰流丽、屋敷童子、幽鬼兔、浮幽樱、儚无水木
